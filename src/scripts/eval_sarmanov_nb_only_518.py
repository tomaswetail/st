#!/usr/bin/env python3
"""100% Sarmanov–NB on official 518 (eval-only). No production change.

Walk-forward fits ``SarmanovNBModel`` per ``(league_external_id, as_of)``
from the live DB. Does not train HGB, does not call ``DixonColesService.fit_league``,
and does not fall back to CSV λ.

```bash
env -u PYTHONPATH /tmp/st-diag-venv/bin/python -m src.scripts.eval_sarmanov_nb_only_518
```
"""
...
from src.calc.sarmanov_nb import SarmanovNBModel
...
NBM_MIN5_LOG_LOSS_CITED = 1.0143
NBM_MIN5_N_SCORED_CITED = 269
NBM_MIN5_MARKET_LOG_LOSS_CITED = 0.9908
SEED_RHO = -0.13
SEED_PHI = 0.05
...
def _phi_stats(models: list[SarmanovNBModel]) -> dict[str, Any]:
...
def _rho_stats(models: list[SarmanovNBModel]) -> dict[str, Any]:
    rhos = [float(model.rho) for model in models]
    if not rhos:
        return {
            "n": 0,
            "min": None,
    ...
    return {
        ...
        "max": max(rhos),
        "n_at_bound": sum(1 for model in models if model.rho_at_bound),
    }
...
def main() -> int:
    ...
    parser.add_argument(
        "--json",
        type=Path,
        default=Path("artifacts/sarmanov_nb_only_518.json"),
    )
    parser.add_argument(
        "--min-team-matches",
        type=int,
        default=None,
        help=(
            "Override SarmanovNBModel min_team_matches. "
    ...
    print(
        ...
        f"fit_rho=True fit_phi=True",
        flush=True,
    )

    base_payload: dict[str, Any] = {
        "kind": "sarmanov_nb_only_518_diagnostic",
        ...
        "cited_nbm_min5": {
            "min_team_matches": 5,
            "nbm_log_loss": NBM_MIN5_LOG_LOSS_CITED,
            "n_scored": NBM_MIN5_N_SCORED_CITED,
            "market_log_loss": NBM_MIN5_MARKET_LOG_LOSS_CITED,
        },
    }
    ...
    cache: dict[tuple[int, date], SarmanovNBModel | None] = {}
    fit_errors: list[str] = []
    try:
    ...
    sarmanov_log_loss = score_log_loss(
    ...
    intersection_sarmanov = score_log_loss(
    ...
    rho_stats = _rho_stats(successful_fits)
    phi_stats = _phi_stats(successful_fits)
    payload = {
        **base_payload,
        ...
        "rho_over_successful_fits": rho_stats,
        "phi_over_successful_fits": phi_stats,
        "csv_lambda_fallback_used": False,
        "sarmanov_log_loss": sarmanov_log_loss,
        "sarmanov_log_loss_4dp": _round4(sarmanov_log_loss),
        "market_same_rows": {
            "n": len(market_same),
        ...
        "intersection_classic_valid_dc": {
            "n": len(intersection),
            "sarmanov_log_loss": intersection_sarmanov,
            "sarmanov_log_loss_4dp": _round4(intersection_sarmanov),
        ...
        "fail_closed_sarmanov_or_market": {
            "label": "not 100% Sarmanov",
            "n": len(fail_closed_labels),
            "n_sarmanov": fail_closed_sarmanov,
        ...
        "cited": {
            ...
            "nbm_min5_log_loss": NBM_MIN5_LOG_LOSS_CITED,
            "nbm_min5_n": NBM_MIN5_N_SCORED_CITED,
            "nbm_min5_market_log_loss": NBM_MIN5_MARKET_LOG_LOSS_CITED,
    ...
    print(
        f"PRIMARY 100% Sarmanov–NB log-loss={_round4(sarmanov_log_loss)} n={n_scored} "
        f"skipped={n_skipped} min_team_matches={min_team_matches} "
        f"csv_lambda_fallback_used=false",
        flush=True,
    )
    print(
        f"cited_nbm_min5 LL={NBM_MIN5_LOG_LOSS_CITED} n={NBM_MIN5_N_SCORED_CITED} "
        f"market={NBM_MIN5_MARKET_LOG_LOSS_CITED}",
        flush=True,
    )
    print(
    ...
    print(f"rho_stats={rho_stats}", flush=True)
    print(f"phi_stats={phi_stats}", flush=True)
    print(
        f"intersection n={len(intersection)} "
        f"sarmanov={_round4(intersection_sarmanov)} "
        f"market={_round4(intersection_market)} "
        f"csv_classic={_round4(intersection_classic)}",
        flush=True,
    )
    print(
        f"fail_closed (not 100% Sarmanov) log-loss={_round4(fail_closed_log_loss)} "
        f"n={len(fail_closed_labels)} sarmanov={fail_closed_sarmanov} "
        f"market={fail_closed_market}",
        flush=True,
    )
    print(
